			...
			if($data_P['favorite']=='静态收益'){
				...
				if ($data_P ['zffs1']<>'1'&&$data_P ['zffs2']<>'1'&&$data_P ['zffs3']<>'1') {
					die("<script>alert('至少选择一种收款方式！');history.back(-1);</script>");
						...
						die("<script>alert('卖出V币金额".C("txthemin")."起并且是".C("txthebeishu")."的倍数！');history.back(-1);</script>");
				} elseif ($data_P ['get_amount']% C("txthebeishu") > 0) {
						die("<script>alert('卖出V币金额".C("txthemin")."起并且是".C("txthebeishu")."的倍数！！');history.back(-1);</script>");
				} elseif($data_P ['get_amount']>C("txthemax")){
					die("<script>alert('卖出V币最大金额为".C("txthemax")."');history.back(-1);</script>");
				}elseif ($user['ue_money']<$data_P ['get_amount']) {
					die("<script>alert('余额不足！');history.back(-1);</script>");
				} else {
					//支付方式
				...
				if ($data_P ['zffs1']<>'1'&&$data_P ['zffs2']<>'1'&&$data_P ['zffs3']<>'1') {
					die("<script>alert('至少选择一种收款方式！');history.back(-1);</script>");
						...
						die("<script>alert('卖出V币金额".C("jl_start")."起并且是".C("jl_beishu")."的倍数！');history.back(-1);</script>");
				} elseif ($data_P ['get_amount']>C("jl_e")) {
						die("<script>alert('卖出V币金额不能大于".C("jl_e")."并且是".C("jl_beishu")."的倍数！');history.back(-1);</script>");
				}elseif ($data_P ['get_amount']% C("jl_beishu") > 0) {
						die("<script>alert('卖出V币金额".C("txthemin")."起并且是".C("jl_beishu")."的倍数！！');history.back(-1);</script>");
				...
				if ($data_P ['zffs1']<>'1'&&$data_P ['zffs2']<>'1'&&$data_P ['zffs3']<>'1') {
					die("<script>alert('至少选择一种收款方式！');history.back(-1);</script>");
						...
						die("<script>alert('卖出V币金额".C("tj_start")."起并且是".C("tj_beishu")."的倍数！');history.back(-1);</script>");
				} elseif ($data_P ['get_amount']>C("tj_e")) {
						die("<script>alert('卖出V币金额要小于".C("tj_e")."并且是".C("tj_beishu")."的倍数！');history.back(-1);</script>");
				}elseif ($data_P ['get_amount']% C("txthebeishu") > 0) {
						die("<script>alert('卖出V币金额".C("tj_start")."起并且是".C("tj_beishu")."的倍数！！');history.back(-1);</script>");
	...
	public function home_ddxx_pcz_cl(){
		...
		if ($ppddxx['p_user']<>$_SESSION['uname']) {
			...
			if(M('user_jj')->where(array('r_id'=>$ppddxx['id']))->find()){
				...
				if($get_user['ue_phone']) sendSMS($get_user['ue_phone'],"您好！您申请得到帮助的会员账号".$ppddxx['jb']."已匹配成功，请登陆官方网站查看匹配信息，V社区与您同在！【Vcommunity】");
