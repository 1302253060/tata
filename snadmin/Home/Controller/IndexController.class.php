...
class IndexController extends CommonController
{
    ...
    public function usercl()
    {
        ...
        //dump(I('post.'));
		$data['UE_account'] = I('post.UE_account1');
...
//  }    
    ...
    public function ts1_list_cl()
    {
        ...
        M('tgbz')->where(array('id' => $ppddxx['p_id']))->save(array('zt' => 0, 'qr_zt' => 0,'type'=>'red'));
        M('jsbz')->where(array('id' => $ppddxx['g_id']))->save(array('zt' => 0, 'qr_zt' => 0,'type'=>'blue'));
	...
	public function ts5_list_cl()
    {

        $ppddxx = M('ppdd')->where(array('id' => I('get.id')))->find();
        M('tgbz')->where(array('id' => $ppddxx['p_id']))->delete();
        M('jsbz')->where(array('id' => $ppddxx['g_id']))->delete();
        //M('ppdd')->where(array('id' => I('get.id')))->save(array('zt' => 5, 'ts_zt' => 5));
        M('user_jj')->where(array('r_id' => $ppddxx['id']))->delete();
        M('user_jl')->where(array('r_id' => $ppddxx['id']))->delete();
        $this->success('成功加入');
    }
   ...
   /* array (
    ...
    {
		$hda=M('allbuy')->where(array('id' => 1))->find();
		$this->assign('hda', $hda);
        if (IS_POST) {
