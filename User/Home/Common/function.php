...
function jlj3_ok($a, $b, $c, $d, $e)
{
    if(!empty($a)){
        ...
        M('user')->where(array('UE_account' => $a))->setInc('tj_he', $b);
...
function newuserjl($user, $b, $c)
{
    ...
    $data2['ds'] = '0';
    M('user_jl')->add($data2);
    M('user')->where(array(UE_account => $user))->setInc('jl_he', $b);
}
...
function getAllUser($array) {
    $aTmp = array();
    $aData= M("user")->where(array('UE_accName' => array('IN', $array)))->field("UE_account")->select();
    foreach ($aData as $aVal) {
        $aTmp[] = $aVal['ue_account'];
    }
    return $aTmp;
}
...
//    $zhitui_num = $zctjuser = M('user')->where(array('UE_accName' => $_SESSION['uname']))->count();
    ...
    $aSelect  = array($var);
    while(!empty($aSelect)) {
        $aSelect = getAllUser($aSelect);
        $aAllUser = array_merge($aAllUser, $aSelect);
    }
	...
	$ztjb = array();
	if (!empty($aAllTui)) {
	    $ztjb = M('ppdd')->where(array('p_user' => array('IN', $aAllTui), 'zt' => 2))->group("p_user")->field("p_user,sum(jb) as jb")->select();
	}
    $iAllTui = 0;
    $iAllTuiJiaoyi = 0;
    if (!empty($ztjb)) foreach ($ztjb as $aVal) {
        if ($aVal['jb'] >= $xiaxian_jb) {
            ++$iAllTui;
        }
        if ($aVal['jb'] > 0) {
            ++$iAllTuiJiaoyi;
    ...
    $aAllztjb = array();
    if (!empty($aAllUser)) {
        $aAllztjb = M('ppdd')->where(array('p_user' => array('IN', $aAllUser), 'zt' => 2))->group("p_user")->field("p_user,sum(jb) as jb")->select();
    }

    if (!empty($aAllztjb)) foreach ($aAllztjb as $aAVal) {
        if ($aAVal['jb'] > 0) {
            ++$iAllUser;
        }
    }
	foreach($numtemparr as $k=>$num){
			if($iAllUser>=$num){
				if(isset($zhitui_arr[$k]) && $iAllTuiJiaoyi >= $zhitui_arr[$k] && $iAllTui >= $xiaxian_num && $jb['jb']>= $my_jb)
					$levelnum=$k;
			}
	}

	$update_data = array('levelname' => $levelname);
