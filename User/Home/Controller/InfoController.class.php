...
class InfoController extends CommonController
{
    ...
    public function xgzlcl()
    {
        if (IS_POST) {
            ...
            if (false) {
                $this->success('买入V币成功后不可修改个人信息和经理人不可自行修改资料!');
            } else {
                $userxx = M('user')->where(array('UE_account' => $_SESSION['uname']))->find();
                ...
                if ($userxx['ue_secpwd'] <> md5($data_P['trade_pwd2'])) {
                    ...
                    $data_up['bit_account'] = $data_P['bit_account'];
    ...
    public function xgyjmmcl()
    {
		...
		die("<script>alert('新密碼6-12個字元,大小寫英文+數字,請勿用特殊詞符！');history.back(-1);</script>");
    ...
    //            $this->ajaxReturn(array('nr' => '新密碼兩次輸入不一致!', 'sf' => 0));
		die("<script>alert('新密碼兩次輸入不一致!');history.back(-1);</script>");
            } elseif ($data_P['ymm'] == $data_P['xmm']) {
		
		die("<script>alert('原密碼和新密碼不能相同!');history.back(-1);</script>");
                //$this->ajaxReturn(array('nr' => '原密碼和新密碼不能相同!', 'sf' => 0));
            } else {
                $addaccount = M('user')->where(array(UE_account => $_SESSION ['uname']))->find();

                if ($addaccount['ue_password'] <> md5($data_P['ymm'])) {
			die("<script>alert('原密碼不正確!');history.back(-1);</script>");			
...
//                                 die("<script>alert('申请后要等".C('jjdjdays')."小时才可以提现哦！');history.back(-1);</script>");
                        ...
                        $this->success("提现转出成功",U('Info/rwhistory'),2);
                        //echo $lx_he;
                    }//
                }
            }
...
///////////////////////////
   ...
   public function nwhistory(){
    ...
    $list2 = $User->where(array('user'=>$_SESSION['uname'],'leixin'=>1))->order('id DESC')->limit($p2->firstRow, $p2->listRows)->select();
    $this->assign('list2', $list2); // 賦值數據集
    $this->assign('page2', $p2->show()); // 賦值分頁輸出
    /////////////////----------------
    ...
    public function qiangdan() {
        $id = I('get.id');
        $JsbzObj = M('jsbz');
        $yz_data = $JsbzObj->where(array('zt'=>0, 'user'=>$_SESSION['uname']))->select();
        if(empty($yz_data)) {
