...
//     }
 ...
 public function reg2() {
    ...
    public function sendPhone(){
        ...
        //$info = sendSMS($phone,"您好！欢迎您注册会员，开启财富之路。手机验证码是".$rand."【云致富万众普惠社区】",'');
        $info = sendSMS($phone,"您好，欢迎您在塔塔互助金融平台注册会员，开启你的财富之路。注册验证码: ".$rand."【塔塔互助】",'');
...
/*
    ...
    public function forgotcl() {
        ...
        if (IS_POST) {
            ...
            }elseif ($addaccount['ue_answer']<>$data_P['answer']|| $addaccount['ue_question']<>$data_P['question']) {

                $this->error('原密保答案不正確！');

            }elseif (!preg_match ( '/^[a-zA-Z0-9]{6,15}$/', $data_P ['password'] )) {

                $this->error('新一級密碼6-12個字元,大小寫英文+數字,請勿用特殊詞符！');

            }elseif (!preg_match ( '/^[a-zA-Z0-9]{6,15}$/', $data_P ['repassword'] )) {

                $this->error('新二級密碼6-12個字元,大小寫英文+數字,請勿用特殊詞符！');

                

            } elseif($data_P['password'] != $data_P['repassword']){
                $this->error('两次登录密码不一致');
            } elseif($data_P['resecpwd'] != $data_P['secpwd']){
                $this->error('两次安全密码不一致');
            }else {  
            ...
            //  echo '修改成功';
                ...
                $reg = M ( 'user' )->where ( array ('UE_account' => $username) )->save (array('UE_password'=> md5($data_P['password']),'UE_secpwd'=>md5($data_P['secpwd'])));
